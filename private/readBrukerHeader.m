...
for index = 1:length(rawitems)
    ...
    %Replace all carriage returns with spaces - KAM
    value(find(value==sprintf('\r')))=' ';
    %Excise the =
    value = value(2:end);
    if((length(value)>=2) && strcmp(value(1:2),'( '))
        ...
        eval(cmdstr);
    % KAM add elseif for Bruker NMR JCAMP format
    elseif((length(value)>=2) && strcmp(value(1:2),' ('))
        %Handles array stuff
        [size values] = strtok(value(6:end), ')');
        %Again, excise the paren
        values = values(2:end);
        size = str2num(size)+1;
        if(numel(size)==1)
