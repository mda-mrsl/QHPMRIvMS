...
ofn = fullfile('data','HPNMRData_Hth83.mat'); % save spectra here
dfn = fullfile('data','HPNMRFits_Hth83.mat'); % save fits here
...
if exist(ofn,'file')
    ...
    Group=struct('Name','Hth83','Data',struct('Indir','tmp','NCells',0,'tstarti',0,'pyri',0,'laci',0));

    % Group 1: Hth83
    
    Group(1).Data(1).Indir  =fullfile('data','20190808-Lai_ATC_cells','2');
    % @ 10:38:13
    Group(1).Data(1).NCells =29.7e6;
    Group(1).Data(1).lastz=5;   %last point before signal;
    Group(1).Data(1).tstarti=7; % Stable by here.
    Group(1).Data(1).pyri=16340;
    Group(1).Data(1).laci=14678;

    Group(1).Data(2).Indir  =fullfile('data','20190816-Lai_ATC_cells','2');
    % @ 11:05:30
    Group(1).Data(2).NCells =26.10e6;
    Group(1).Data(2).lastz=5;   
    Group(1).Data(2).tstarti=8; 
    Group(1).Data(2).pyri=16337;
    Group(1).Data(2).laci=14674;

    Group(1).Data(3).Indir  =fullfile('data','20190816-Lai_ATC_cells','5');
    % @ 14:50:56
    Group(1).Data(3).NCells =26.87e6;
    Group(1).Data(3).lastz=4;   
    Group(1).Data(3).tstarti=6; 
    Group(1).Data(3).pyri=16337;
    Group(1).Data(3).laci=14675;

    Group(1).Data(4).Indir  =fullfile('data','20190816-Lai_ATC_cells','7');
    % @ 16:11:18
    Group(1).Data(4).NCells =18.50e6;
    Group(1).Data(4).lastz=6;   
    Group(1).Data(4).tstarti=8; 
    Group(1).Data(4).pyri=16336;
    Group(1).Data(4).laci=14670;

    Group(1).Data(5).Indir =fullfile('data','20190906-Lai_ATC_cells','7');
    % @ 16:42:27
    Group(1).Data(5).NCells=15.59e6;
    Group(1).Data(5).lastz=8;
    Group(1).Data(5).tstarti=12;  % Note big hiccup before this point
    Group(1).Data(5).pyri=16336;
    Group(1).Data(5).laci=14673;
    ...
    for gn=1:NG
        for dn=1:ND
            ...
            plot(taxis,spyr,'g',taxis,slac,'b',taxis(Group(gn).Data(dn).tstarti),spyr(Group(gn).Data(dn).tstarti),'ro',...
                taxis(Group(gn).Data(dn).tstarti),slac(Group(gn).Data(dn).tstarti),'ro','linewidth',2)
            axis tight;grid on
...
%Volume of a cell that is 22um in diameter => r = 11e-4cm:
vhtcell = 4/3*pi*((11e-4)^3); % expressed in mL
...
if strcmp(refit,'Yes')
    ...
    for gn=1:NG
        for dn=1:ND
            ...
            fdv.Name='PK Analysis - Hth83 ATC';
            ...
            for kpli=1:nkpls
                ...
                %Initialize 'nuisance parameters' - known or estimated
                ...
                if parallelize
                    ...
                    parfor ii=(oldruns+1):(totruns)
                        ...
                        try
                            ...
                            fprintf('Warning! Exception Caught: %s.\n',ME.identifier)
                            resid=Inf;
