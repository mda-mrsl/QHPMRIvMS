...
ofn = fullfile('data','HPNMRData_PC3.mat'); % save spectra here
dfn = fullfile('data','HPNMRFits_PC3.mat'); % save fits here
...
if exist(ofn,'file')
    ...
    Group=struct('Name','PC3','Data',struct('Indir','tmp','NCells',0,'tstarti',0,'pyri',0,'laci',0));

    % Group 1: PC3 
    
    %Note: viability 74%
    Group(1).Data(1).Indir  =fullfile('data','20211005-Lai_PC3_lncap_hth83_cells','1');
    Group(1).Data(1).NCells =(2.36e7)*0.9; % final cell density (cell/mL) * vol (mL)
    Group(1).Data(1).lastz=9;   %last point before signal;
    Group(1).Data(1).tstarti=12; % Stable by here.
    Group(1).Data(1).pyri=16348;
    Group(1).Data(1).laci=14689;

    % Viability 62%
    Group(1).Data(2).Indir  =fullfile('data','20211005-Lai_PC3_lncap_hth83_cells','2');
    Group(1).Data(2).NCells =(1.76e7)*0.9; % final cell density (cell/mL) * vol (mL)
    Group(1).Data(2).lastz=9;   %last point before signal;
    Group(1).Data(2).tstarti=13; % Stable by here.
    Group(1).Data(2).pyri=16344;
    Group(1).Data(2).laci=14687;
    
    %Viability 78%
    Group(1).Data(3).Indir  =fullfile('data','20211006-Lai_PC3_lncap_hth83_cells','6');
    Group(1).Data(3).NCells =(1.196e7)*0.9; % final cell density (cell/mL) * vol (mL)
    Group(1).Data(3).lastz=6;   %last point before signal;
    Group(1).Data(3).tstarti=9; % Stable by here.
    Group(1).Data(3).pyri=16347;
    Group(1).Data(3).laci=14690;
...
if strcmp(refit,'Yes')
    ...
    for gn=1:NG
        for dn=1:ND
            ...
            fdv.Name='PK Analysis - PC3 Prostate';
