...
pk1pcpostfn=fullfile('data','HPMRIPK_post_1PC.20250917.mat');
pk2pcpostfn=fullfile('data','HPMRIPK_post_2PC.20250917.mat');
pk3pcpostfn=fullfile('data','HPMRIPK_post_3PC.20250917.mat');
...
nlacaxis(4)=6; % fix problem - y axis scales when x label set..?
axis(nlacaxis);
if textflag
...
load(pk2pcpostfn,'fdvb','kplbfits','kplbresids','kplvec');
...
registeredoverlay2(valnames,kplb(:,:,sno).*kplbmask(:,:,sno),hph,1,...
    bgims(:,:,bgi(sno)),bgh,zrx,zry,1);
set(gcf,'position',figpos)
ofn='Fig7c3v3-Post-kplb-overlay';
if ~exist([ofn '.tif'],'file')||(resave==1)
    exportgraphics(gcf,[ofn '.tif']);
    exportgraphics(gcf,[ofn '.png']);
end
...
set(gca,'fontsize',12)
set(gcf,'position',figposh)
ofn='FigS332v3-Post-kplb-histogram';
if ~exist([ofn '.eps'],'file')||(resave==1)
    print(ofn,'-depsc');
    exportgraphics(gcf,[ofn '.png']);
end
...
load(pk3pcpostfn,'fdvc','kplcfits','kplcresids','kplvec');
    ...
    for jj=1:size(kplcfits,2)
        for kk=1:size(kplcfits,3)
            if kplcmask(ii,jj,kk)
                ...
                end %calculation of kplc                kves=squeeze(kplcfits(ii,jj,kk,:,kvei))';
            end % mask
        end %slice
    end %rows
end %columns
...
registeredoverlay2(valnames,kplc(:,:,sno).*kplcmask(:,:,sno),hph,1,...
    bgims(:,:,bgi(sno)),bgh,zrx,zry,1);
set(gcf,'position',figpos)
ofn='Fig7d3v3-Post-kplc-overlay';
if ~exist([ofn '.tif'],'file')||(resave==1)
    exportgraphics(gcf,[ofn '.tif']);
    exportgraphics(gcf,[ofn '.png']);
end
...
set(gca,'fontsize',12)
set(gcf,'position',figposh)
ofn='FigS342v3-Post-kplc-histogram';
if ~exist([ofn '.eps'],'file')||(resave==1)
    print(ofn,'-depsc');
    exportgraphics(gcf,[ofn '.png']);
end

save('QHPMRIPost.mat','kpla','kplamask','nLac','nlacmask',...
