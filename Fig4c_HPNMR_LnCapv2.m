...
load(fullfile('data','HPNMRFits_LnCap.mat')); 
NGrp=length(Group);
...
for gn=1:NGrp
    for dn= 2%1:NSamp % 2
        ...
        fdv.Name=sprintf('LnCap Sample %d',dn);
        ...
        %Set up plots:
        fig4ch=figure(430+dn); % Fig 4c
        set(gcf,'Name',sprintf('NMR - %s',Group.Name))
        ...
        %Plot!
        nexttile
        f4c1h=plot(fdv.taxis(plotveci==1),fdv.data(1,plotveci==1),'gx',...
        ...
        exportgraphics(gcf,sprintf('Fig4c-NMR-%s.png',Group.Name),'Resolution',352); % default res 150
